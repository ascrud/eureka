...
public class EurekaUpStatusResolver {
    ...
    private static Logger logger = LoggerFactory.getLogger(EurekaUpStatusResolver.class);
    ...
    public void onStatusChange(StatusChangeEvent event) {
        logger.info("Eureka status changed from {} to {}", event.getPreviousStatus(), event.getStatus());
    ...
    public void init() {
        try {
            ...
            logger.info("Initial status set to {}", currentStatus);
            eventBus.registerSubscriber(this);
        } catch (InvalidSubscriberException e) {
            logger.error("Error registring for discovery status change events.", e);
        }
    }
